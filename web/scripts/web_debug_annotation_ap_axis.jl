...
function web_debug_annotation_ap_axis(datasets = datasets)
            ...
            map(collect(keys(datasets))) do k
                ...
                DOM.ul(
                    map(collect(keys(datasets[k]))) do i
                        DOM.li(DOM.a(datasets[k][i].path, href="/debug_annotation_ap_axis/$k/$i"))
                    end
                )
            end
        )
    ...
    server = Server(
        "shroff-data.int.janelia.org", 9180;
        proxy_url="https://shroff-data.int.janelia.org/debug_annotation_ap_axis/"
    )
